...
module "catalogue" {
  ...
  name                   = "${local.name}-${var.tags.Component}-ami_instance"
...
resource "null_resource" "catalogue" {
  # Changes to any instance of the cluster requires re-provision ing
  triggers = {
...
resource "aws_ec2_instance_state" "ami_instance_stop" {
  instance_id = module.catalogue.id
  state       = "stopped"
...
resource "aws_ami_from_instance" "catalogue_ami" {
  ...
  depends_on         = [aws_ec2_instance_state.ami_instance_stop]
}

resource "null_resource" "catalogue_ami_instance_delete" {
  # Changes to any instance of the cluster requires re-provision ing
  triggers = {
    instance_id = module.catalogue.id
  }
