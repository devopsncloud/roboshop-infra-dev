resource "aws_cloudfront_distribution" "roboriya" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.roboriya.domain_name
        zone_id = aws_cloudfront_distribution.roboriya.hosted_zone_id
      }
    }
  ]
}